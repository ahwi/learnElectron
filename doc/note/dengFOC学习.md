...
* 实验1：测试闭环速度代码 （只有 P 环的情况）
* 实验2：测试闭环速度代码（加入 I 环的情况）
* 两个实验对比：在有负载的情况下，加入I环能根据负载进行动态调整
...
void loop() 
{
  //设置速度环PID （只有P环）
   ...
   // //设置速度环PID （PI环）
   // DFOC_M0_SET_VEL_PID(0.001,0.01,0,0);
  //设置速度
...
<img src="dengFOC学习.assets/image-20250909155315292.png" alt="image-20250909155315292" style="zoom:50%;" />

在之前的课程中，我们已经完成了 位置闭环、速度闭环、电压力环 的学习，但是在之前代码中的FOC力矩环却依然是以开环的电压力矩通过直接设置Uq来实现电机力矩输出的。在这种情况下，由于电机的力矩没有传感器进行检测，因此电机的力矩实际上是开环的。

为了更精确的控制电机，需要对这个力矩进行精确的闭环控制，以实现精确的力矩定量控制。

求出电机力矩并进行力矩闭环：电流传感器的电流输出，在经过一定的运算后可以转换为力矩值。
...
电流环的作用：通过电流反馈，实现更精确的力矩控制

电流环的原理：电机三相线的电流经过一定的算法运算后，可以转换为力矩（观测到的力矩），得到观测值后，我们就可以进行力矩闭环（用期望力矩减去观测到的力矩，得到误差并且把它放在PID里面进行输出）。

#### 6.2.1 算法原理

* 假设我们能检测出电机三相线中其中两相电流$i_a$、$i_b$，那么就可以通过克拉克变换求出$I_\alpha$和$I_\beta$
* 进一步，通过帕克变换，$I_\alpha$和$I_\beta$可以换算成$I_q$和$I_d$

* $I_q$在一定程度上可以代表电机力矩，只需要知道电机的KV值，就能够通过下面的式子把$I_q$换算成电机力矩：

  $$Torque [N.m] = 8.27 * I_q [A] / KV.$$

* 想要控制力矩，只要$I_q$值能够达到我们设定的期望$I_{qRef}$值就行，把它们两个的差值代入到PID算法中进行电流力矩的闭环：

  $$U_q = PID(I_{qRef} - I_q)$$

  * $I_q$：是传感器观测的电机q轴电流（观测力矩）
  * $I_{qRef}$：是期望的q轴电流（期望力矩）
  * 两者经过PID算法运算后输出能对电机进行控制的$U_q$值，驱动电机运动

在有电流力矩反馈的情况下，电机最终能够稳定在期望的力矩之中，这就是电流闭环的原理。
