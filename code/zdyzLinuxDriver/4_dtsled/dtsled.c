...
* @param - filp : 设备文件，file结构体有个叫做 private_data 的成员变量
                  一般在open的时候将 private_data指向设备结构体
* @return       : 0 成功；其他 失败
*/
...
*/
static int led_read(struct file *filp, char __user *buf, 
...
static int __init led_init(void)
{
    ...
    proper = of_find_property(dstled.nd, "compatible", NULL);
    if(proper == NULL) {
    ...
    /* 2. 使能GPIO1时钟 */
    val = readl(IMX6U_CCM_CCR1);
    val &= ~(3 << 26); // 清除以前的设置
    val |= ~(3 << 26); // 设置新值
    writel(val, IMX6U_CCM_CCR1);
