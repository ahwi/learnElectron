...
#define KEY_NODE_PATH   "/mykey"            /* 设备上路径 */
#define KEY_NODE_PROPERTY_GPIO "myled-gpio" /* gpio属性 */  
...
* @return           : 中断执行结果
*/
struct irqreturn_t key0_handler(int irq, void *dev_id)
{
...
static int keyio_init(void)
{
    ...
    keyinputdev.nd = of_find_node_by_name(KEY_NODE_PATH);
    ...
    for(i = 0; i < KEY_NUM; i++) {
        keyinputdev.irqkeydesc[i].gpio = 
            of_get_named_gpio(keyinputdev.nd, KEY_NODE_PROPERTY_GPIO, i);
    ...
    for(i = 0; i < KEY_NUM; i++) {
        ret = request_irq(keyinputdev.irqkeydesc[i].irqnum,
                          ...
                          IRQF_TRIGGER_FAILING|IRQF_TRIGGER_RISING,
    ...
    keyinputdev.inputdev->evbit[0] = BIT_MASK(EV_KEY) | BIT_MASK(EV_REP);
    input_set_capabiliby(keyinputdev.inputdev, EV_KEY, KEY_0);
